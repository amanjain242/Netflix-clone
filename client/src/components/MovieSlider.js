...
export default React.memo(function MovieSlider ({ data, title })  {
  ...
  const handlerDirection = (direction)=>{
    ...
    >
      ...
      <div className="wrapper">
        <div className={`slider-action left ${!controlVisibility ? 'none' : ''}`}>
          <AiOutlineLeft  onClick={()=> handlerDirection('left')}/>
        </div>
        ...
        <div className={`slider-action right ${!controlVisibility ? 'none' : ''}`}>
          <AiOutlineRight onClick={()=> handlerDirection('right')}/>
        </div>
      </div>
...
h1{
  margin-left: 5px;
  font-size: 25px;
  font-family: 'Franklin Gothlic Medium', 'Arial Narrow', Arial, sans-serif;
  color: white;
  font-weight: 820px;
}
.wrapper{
